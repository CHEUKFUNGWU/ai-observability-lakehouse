...
def test_run_scale_passes_quarantine_path_and_returns_quarantine_rows(tmp_path, monkeypatch):
    ...
    def fake_run_backfill(*, spark, input_path, quarantine_output, write_to_paimon):
        assert spark is not None
        assert input_path == Path("data/benchmarks/10/raw/events.jsonl")
        assert write_to_paimon is False
        captured["quarantine_output"] = quarantine_output
        quarantine_output.mkdir(parents=True, exist_ok=True)
        (quarantine_output / "part-000.parquet").write_bytes(b"data")
        return {"dwd_rows": 10, "dws_rows": 4, "quarantine_rows": 3}
    ...
    monkeypatch.setattr(run_benchmark, "run_backfill", fake_run_backfill)

    result = run_benchmark.run_scale(10)

    assert result["quarantine_rows"] == 3
    assert result["dwd_rows"] == 10
    assert result["dws_rows"] == 4
...
def test_render_markdown_includes_quarantine_column():
    ...
    assert "| 10 | 1.0 | 4.0 | 10 | 3 | 5 | n/a |" in markdown
    assert "Backfill Duration (s)" in markdown
