...
DROP TABLE IF EXISTS ai_observability.dwd_ai_agent_orchestration_di;
...
CREATE TABLE IF NOT EXISTS ai_observability.dwd_ai_agent_orchestration_di
(
    `date` DATE NOT NULL,
    ...
    handoff_latency_ms INT NOT NULL,
    status VARCHAR(32) NOT NULL,
    created_at DATETIME(3) NOT NULL
)
DUPLICATE KEY(`date`, orchestration_id)
PARTITION BY RANGE(`date`) ()
DISTRIBUTED BY HASH(orchestration_id) BUCKETS 4
PROPERTIES (
    "replication_num" = "1",
    "dynamic_partition.enable" = "true",
    "dynamic_partition.time_unit" = "MONTH",
