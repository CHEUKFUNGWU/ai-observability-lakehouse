...
def build_spark_session() -> SparkSession:
    return (
        SparkSession.builder.appName("ai-observability-ads-agent-daily-metrics")
        .master("local[*]")
...
def build_agent_daily_metrics(runs: DataFrame, spans: DataFrame) -> DataFrame:
    ...
    return (
        run_metrics.join(span_metrics, on=["date", "agent_id"], how="left")
        .withColumn("success_rate", F.round(F.col("success_count") / F.col("run_count"), 4))
        .withColumn("error_rate", F.round(F.col("error_count") / F.col("run_count"), 4))
        .withColumn(
            "span_failure_rate",
...
def main() -> None:
    ...
    try:
        ...
        print(f"Built ADS agent daily metrics: {args.output} ({metrics.count()} rows)")
    finally:
        spark.stop()
