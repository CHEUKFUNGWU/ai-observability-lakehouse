...
from app.warehouse_contract import build_llm_feature_env_request_1d_projection
from app.logging_utils import get_logger, log_info
from scripts.spark_utils import build_spark_session


DEFAULT_INPUT_PATH = Path("data/warehouse/dwd/dwd_ai_llm_request_di/events.parquet")
DEFAULT_OUTPUT_PATH = Path("data/warehouse/dws/dws_ai_llm_feature_env_request_1d.parquet")
...
def build_feature_env_daily_metrics(events: DataFrame) -> DataFrame:
    return build_llm_feature_env_request_1d_projection(
        events.groupBy("date", "app_name", "feature_name", "model_name", "environment").agg(
...
def main() -> None:
    ...
    spark = build_spark_session("ai-observability-dws-feature-env-daily-metrics")

    try:
        metrics = build_feature_env_daily_metrics(load_dwd_events(spark, args.input))
        write_dws_metrics(metrics, args.output)
        log_info(LOGGER, "dws_llm_feature_env_metrics_written", rows=metrics.count(), output=str(args.output))
    finally:
        spark.stop()
