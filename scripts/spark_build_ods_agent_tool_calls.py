...
def main() -> None:
    ...
    spark = build_spark_session("ai-observability-ods-agent-tool-calls")
    try:
        ...
        log_info(LOGGER, "ods_agent_tool_calls_written", output=str(args.output), rows=ods_events.count())
    finally:
        spark.stop()
